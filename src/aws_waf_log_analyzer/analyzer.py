...
class WAFLogAnalyzer:
    ...
    def __init__(self, bucket_name: Optional[str] = None, region: str = 'eu-west-1', 
    ...
    def initialize_s3_client(self):
        ...
        try:
            ...
            self.console.print("[red]Error: AWS credentials not found. Please configure your AWS credentials.[/red]")
            self.console.print("Run 'aws configure' or set environment variables AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY")
            raise
    ...
    def discover_waf_buckets(self) -> List[str]:
        ...
        try:
            ...
            for bucket in response['Buckets']:
                ...
                if any(keyword in bucket_name.lower() for keyword in ['waf', 'logs', 'webacl']):
    ...
    def filter_s3_objects_by_time(self, bucket_name: str, prefix: str, 
    ...
    def download_and_decompress_file(self, bucket_name: str, key: str) -> Iterator[Dict[str, Any]]:
    ...
    def process_files_parallel(self, bucket_name: str, file_keys: List[str], 
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_key = {
                executor.submit(self.download_and_decompress_file, bucket_name, key): key 
    ...
    def format_output_table(self, results: List[Dict[str, Any]]) -> None:
        ...
        if not results:
            self.console.print("[yellow]No results found matching the criteria.[/yellow]")
            return
        ...
        for entry in results[:max_display]:
            ...
            table.add_row(
                ...
                entry.get('uri', 'N/A')[:50] + '...' if len(entry.get('uri', '')) > 50 else entry.get('uri', 'N/A'),
                entry.get('host', 'N/A')
            )
    ...
    def format_output_csv(self, results: List[Dict[str, Any]], output_file: str) -> None:
        ...
        fieldnames = [
            'timestamp', 'action', 'terminating_rule_id', 'terminating_rule_type',
            'client_ip', 'country', 'host', 'method', 'uri', 'http_version',
            'request_id', 'body_size', 'webacl_id', 'http_source_name', 'http_source_id'
        ]
    ...
    def format_output_json(self, results: List[Dict[str, Any]], output_file: str) -> None:
...
Errors: {self.stats['errors']}""",
    ...
    def analyze_enhanced(self, time_range: str, filters: Dict[str, Any], 
        ...
        if not buckets:
            self.console.print("[red]No WAF log buckets found. Please specify a bucket name.[/red]")
            return
        ...
        for bucket_name in buckets:
            ...
            for prefix in prefixes:
                ...
                file_keys = self.filter_s3_objects_by_time(bucket_name, prefix, start_time, end_time)
                self.stats['total_files'] += len(file_keys)
                
                if file_keys:
                    self.console.print(f"Found {len(file_keys)} files in {bucket_name}/{prefix}")
                    
                    # Process files
                    results = self.process_files_parallel(bucket_name, file_keys, filters)
        ...
        if output_format == 'table':
            ...
            if not output_file:
                ...
                output_file = f"waf_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            self.format_output_csv(all_results, output_file)
        elif output_format == 'json':
            if not output_file:
                from datetime import datetime
                output_file = f"waf_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
