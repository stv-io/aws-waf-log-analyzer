...
def parse_time_range(time_input: str) -> Tuple[datetime, datetime]:
    ...
    natural_patterns = {
        r'last (\d+) (hour|hours|hr|hrs)': lambda m: now - timedelta(hours=int(m.group(1))),
        r'last (\d+) (day|days)': lambda m: now - timedelta(days=int(m.group(1))),
        r'last (\d+) (minute|minutes|min|mins)': lambda m: now - timedelta(minutes=int(m.group(1))),
...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    if output_config['default_format'] not in valid_formats:
        raise ValueError(f"Invalid default output format: {output_config['default_format']}")
