...
from .utils import (
    parse_time_range, setup_logging, load_config, get_default_config,
...
@click.command()
@click.option('--bucket', '-b', help='S3 bucket name (auto-discovered if not specified)')
@click.option('--time-range', '-t', default='last 1 hour', 
...
@click.option('--action', '-a', type=click.Choice(['BLOCK', 'ALLOW', 'COUNT']), help='Filter by WAF action')
...
def main(bucket: Optional[str], time_range: str, host: Optional[str], 
         action: Optional[str], output: str, output_file: Optional[str],
         region: str, verbose: bool, config: Optional[str],
         log_level: Optional[str], log_file: Optional[str], version: bool) -> None:
...
@config.command()
@click.option('--output', '-o', type=click.Path(), default='waf-analyzer-config.yaml',
